...
export default function Day() {
  ...
  function containerWidth(): number {
    const w = swipeContainerRef.current?.clientWidth ?? window.innerWidth;
    return Math.max(1, w);
  }
  ...
  function animateToDay(direction: "prev" | "next") {
    ...
    window.setTimeout(() => {
      // Immediately jump to the opposite side with new content
      setIsTransitioning(false);
      setDragX(direction === "prev" ? -w : w);
      setActiveKey(direction === "prev" ? prevKey : nextKey);
      // Next frame animate back to center
  ...
  function onTouchEnd() {
    ...
    const w = containerWidth();
    const threshold = Math.max(80, Math.min(180, Math.floor(w * 0.22)));
    if (dragX > threshold) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div className="m-1 flex flex-col gap-3">
            <div className="flex items-center gap-3 flex-1 w-full sm:w-auto sm:flex-0">
              <Button aria-label="Previous day" size="icon" onClick={goToPrev}>
        ...
        <Progress
          className="w-full mt-3 mb-4 sm:w-64 p"
        ...
        <div className="border-2 border-border w-full h-px my-3" />
